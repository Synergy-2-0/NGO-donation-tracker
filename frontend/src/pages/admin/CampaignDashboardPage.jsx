...
function StatusBadge({ status }) {
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[9px] font-extrabold uppercase tracking-widest border transition-all ${cfg.className}`}>
            {cfg.icon}
            {cfg.label}
        </span>
    );
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-16 animate-fadeIn text-left pt-6">
            <div className="relative overflow-hidden bg-tf-dark rounded-[32px] p-8 md:p-10 shadow-2xl">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <p className="text-[10px] font-extrabold uppercase tracking-[.2em] text-slate-500 mb-2 ">{t('campaign_registry.operational_hub')}</p>
                        <h2 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight ">{t('campaign_registry.title_1')} <span className="text-tf-primary not-">{t('campaign_registry.title_2')}</span></h2>
                        <p className="text-slate-400 text-sm mt-2 max-w-xl font-medium ">{t('campaign_registry.subtitle')}</p>
                        {user?.role === 'ngo-admin' && (
                            <Link 
                                to="/admin/campaigns/create" 
                                className="flex items-center gap-2 px-8 py-4 bg-tf-primary text-white rounded-[24px] text-xs font-extrabold uppercase tracking-widest hover:bg-white hover:text-slate-900 transition-all hover:shadow-[0_0_30px_rgba(220,38,38,0.3)] active:scale-95 group mt-6 "
                            >
                ...
                {stats.map((s, idx) => (
                    <div key={idx} className="bg-white rounded-[32px] border border-slate-100 p-7 flex items-center gap-5 shadow-sm hover:shadow-md transition-shadow group ">
                        ...
                        <div className="min-w-0">
                            <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-1 truncate">{s.label}</p>
                            <h4 className="text-xl font-extrabold text-slate-900 truncate tracking-tight">{s.value}</h4>
                        </div>
                    </div>
                ))}
            </div>
                ...
                <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input 
                            ...
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl pl-12 pr-4 py-3 text-sm font-semibold focus:outline-none focus:ring-4 focus:ring-tf-primary/5 focus:border-tf-primary/20 transition-all "
                        />
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <select
                            ...
                            className="bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3 text-[10px] font-extrabold uppercase tracking-widest text-slate-600 focus:outline-none focus:ring-4 focus:ring-tf-primary/5 transition-all outline-none appearance-none cursor-pointer pr-10 relative "
                ...
                <div className="overflow-x-auto min-h-[400px]">
                    <table className="w-full border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50">
                                <th className="px-8 py-5 text-left text-[10px] font-extrabold uppercase tracking-[.2em] text-slate-400  font-sans">{t('campaign_registry.table.identity')}</th>
                                <th className="px-8 py-5 text-left text-[10px] font-extrabold uppercase tracking-[.2em] text-slate-400  font-sans">{t('campaign_registry.table.progress')}</th>
                                <th className="px-8 py-5 text-center text-[10px] font-extrabold uppercase tracking-[.2em] text-slate-400  font-sans">{t('campaign_registry.table.level')}</th>
                                <th className="px-8 py-5 text-right text-[10px] font-extrabold uppercase tracking-[.2em] text-slate-400  font-sans">{t('campaign_registry.table.actions')}</th>
                            </tr>
                        </thead>
                            ...
                            {paginatedCampaigns.length > 0 ? paginatedCampaigns.map((c) => (
                                <tr key={c._id} className="group hover:bg-slate-50/50 transition-all duration-200">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-5  text-left">
                                            ...
                                            <div className="min-w-0">
                                                <p className="text-sm font-extrabold text-slate-800 truncate tracking-tight group-hover:text-tf-primary transition-colors">{c.title}</p>
                                                <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mt-1">ID: {c._id.slice(-8)} HUB</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        ...
                                        <div className="flex items-center justify-end gap-2 ">
                                            ...
                                            {c.status === 'draft' && user?.role === 'ngo-admin' && (
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-tf-primary/10 text-tf-primary rounded-full text-[10px] font-extrabold uppercase tracking-widest hover:bg-tf-primary hover:text-white transition-all shadow-sm border border-tf-primary/20 flex items-center gap-2 active:scale-95 "
                                                >
                                            ...
                                            {(c.status === 'draft' || c.status === 'pending') && user?.role === 'admin' && (
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-emerald-500 text-white rounded-full text-[10px] font-extrabold uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl shadow-emerald-500/10 border border-emerald-400 flex items-center gap-2 active:scale-95 "
                                                >
                                            ...
                                            <Link
                                                ...
                                                className="px-4 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-extrabold uppercase tracking-widest hover:bg-tf-primary transition-all shadow-xl shadow-slate-900/10 active:scale-95 flex items-center gap-2 "
                                            >
                                ...
                                <tr>
                                    <td colSpan="4" className="px-8 py-20 text-center ">
                                        ...
                                        <p className="text-slate-400 font-extrabold text-sm uppercase tracking-widest ">{t('campaign_registry.no_results')}</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
                    ...
                    {totalPages > 1 && (
                        <div className="p-8 border-t border-slate-50 flex items-center justify-between bg-slate-50/10 ">
                            <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Page {currentPage} of {totalPages}</span>
