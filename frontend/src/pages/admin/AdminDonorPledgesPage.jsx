      ...
      <div className="relative p-12 lg:p-16 bg-tf-purple rounded-[4rem] overflow-hidden shadow-2xl group text-white border border-white/5">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10">
           <div className="space-y-4">
              ...
              <h2 className="text-3xl lg:text-4xl font-black tracking-tighter uppercase italic tracking-tight">Pledge <span className="text-tf-primary">Commitment </span> Hub</h2>
           </div>
           <div className="bg-white/5 border border-white/10 backdrop-blur-md px-10 py-5 rounded-[2.5rem] flex items-center gap-12 text-center">
              <div>
                 ...
                 <p className="text-2xl font-black italic text-tf-primary">{pledges.length}</p>
              </div>
      ...
      <div className="flex flex-wrap gap-5 items-center bg-white rounded-[3.5rem] border border-slate-100 p-4 shadow-sm">
        <div className="flex-1 min-w-[300px] relative h-16 group">
           <input
             ...
             className="w-full h-full bg-slate-50 border border-slate-100 rounded-full px-12 text-[13px] font-bold text-tf-purple placeholder:text-slate-300 focus:outline-none focus:border-tf-primary transition-all shadow-inner"
           />
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((pledge) => (
                  <tr key={pledge._id} className="group hover:bg-slate-50 transition-all cursor-default">
                    <td className="px-10 py-10">
                      <div className="flex items-center gap-6">
                         ...
                         <div className="space-y-1">
                            <span className="block text-[15px] font-black text-tf-purple tracking-tight group-hover:text-tf-primary transition-colors">{pledge.donorId?.userId?.name || 'Partner Member'}</span>
                    ...
                    <td className="px-10 py-10">
                       <div className="space-y-1">
                          <span className="text-xl font-black text-tf-purple tracking-tighter italic tabular-nums group-hover:text-tf-primary transition-colors">LKR {Number(pledge.amount).toLocaleString()}</span>
                    ...
                    <td className="px-10 py-10">
                       <button
                         ...
                         className="px-8 py-3 bg-white border-2 border-slate-100 hover:border-tf-primary hover:text-tf-primary text-tf-purple text-[10px] font-black uppercase tracking-[0.2em] rounded-xl transition-all shadow-sm hover:shadow-xl hover:shadow-tf-primary/10 active:scale-95"
                       >
                         View Member
                       </button>
                    </td>
