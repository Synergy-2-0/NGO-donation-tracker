...
export default function TransactionsPage() {
  ...
  if (loading && transactions.length === 0) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
      <LoadingSpinner />
      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 animate-pulse">Synchronizing Global Ledger Hub...</p>
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-32 pt-6">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-10">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/5 rounded-full border border-white/10 backdrop-blur-sm">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Transaction Intelligence Hub</span>
            </div>
          ...
          <div className="flex gap-4">
            <button className="px-10 py-5 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:border-orange-500 transition-all flex items-center gap-3 text-slate-900 shadow-xl shadow-white/5 active:scale-95">
      ...
      <div className="premium-surface p-6 flex flex-col md:flex-row items-center gap-6">
         ...
         <div className="flex items-center gap-4 w-full md:w-auto">
            <button className="px-8 py-5 bg-slate-50 border border-slate-100 rounded-3xl text-[10px] font-black uppercase tracking-widest text-slate-600 flex items-center gap-3 hover:bg-slate-100 transition-all">
      ...
      <div className="premium-surface overflow-hidden">
        <div className="overflow-x-auto min-h-[400px]">
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] text-slate-400 uppercase tracking-widest font-black border-b border-slate-50 bg-slate-50/50">
              ...
              {filtered.length > 0 ? (
                  ...
                  <tr key={tx._id} className="group hover:bg-slate-50/50 transition-colors">
                    <td className="px-10 py-10">
                        <div className="flex items-center gap-5">
                           ...
                           <div>
                              <p className="font-mono text-sm text-slate-900 font-black tracking-tighter">
                                #{tx.payHereOrderId || tx._id.slice(-12).toUpperCase()}
                              </p>
                              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1 italic">Registry Entry Hub</p>
                           </div>
                        </div>
                    </td>
                    <td className="px-10 py-10">
                        <div className="space-y-2">
                           <p className="text-base font-black text-slate-950 line-clamp-1 italic">{tx.campaignId?.title || 'General Treasury Funds'}</p>
                           <div className="flex items-center gap-2">
                              <div className="w-5 h-5 rounded-full bg-slate-100 flex items-center justify-center">
                                 <FiUsers size={10} className="text-orange-500" />
                              </div>
                              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{tx.donorId?.name || 'Anonymous Institutional Donor'}</span>
                           </div>
                        </div>
                    </td>
                    <td className="px-10 py-10">
                        <div className="space-y-1">
                           ...
                           <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Finalized Hub</span>
                           </div>
                        </div>
                    </td>
                    <td className="px-10 py-10">
                        <p className="text-lg font-black text-slate-950 tabular-nums leading-none mb-1">
                    ...
                    <td className="px-10 py-10">
                        <div className="flex justify-center">
                          <span className={`px-5 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest border shadow-sm transition-all group-hover:shadow-md ${statusBadgeStyle[tx.status] || 'bg-slate-50 text-slate-400'}`}>
                            {tx.status} Hub
                          </span>
                        </div>
                    </td>
                ...
                <tr>
                  <td colSpan="6" className="px-10 py-40 text-center">
                        ...
                        <h3 className="text-slate-300 font-black text-lg uppercase tracking-[0.2em] italic">Zero Ledger Intelligence</h3>
            ...
            >
              <div className="p-12 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                 <div className="flex items-center gap-6">
                    ...
                    <div>
                       <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-2 italic">Institutional Payment Verification Registry Hub</p>
                ...
                <div className="space-y-12 text-left">
                  <div className="space-y-6">
                    ...
                    <div className="p-8 bg-slate-50 rounded-[40px] space-y-6 border border-slate-100">
                       <div>
                          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Donor Identity</p>
                          <p className="text-2xl font-black text-slate-900 italic">{selectedTx.donorId?.name || 'Anonymous Operational Donor'}</p>
                       </div>
                       <div>
                          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Authenticated Relay</p>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="p-8 bg-slate-900 rounded-[40px] text-white relative overflow-hidden group">
                       ...
                       <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Target Campaign Registry</p>
                       <p className="text-2xl font-black text-white italic leading-tight mb-4">{selectedTx.campaignId?.title || 'Operational Emergency Fund Hub'}</p>
                       <div className="flex justify-between items-end pt-4 border-t border-white/10">
                          <div>
                             <p className="text-[8px] font-black uppercase tracking-widest text-slate-500 mb-1">Relay Status</p>
                             <p className="text-base font-black text-emerald-400 italic">VERIFIED SYNC</p>
                          </div>
                          <p className="text-[8px] font-black uppercase tracking-widest text-slate-500">Class A Protocol</p>
                       </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-12 text-left">
                  <div className="space-y-6">
                    ...
                    <div className="premium-surface p-8 space-y-6">
                       <div className="grid grid-cols-2 gap-8">
                          <div>
                             <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Sync Liquidity</p>
                             <p className="text-2xl font-black text-slate-900 tabular-nums leading-none italic">{selectedTx.currency} {selectedTx.amount?.toLocaleString()}</p>
                          </div>
                          <div>
                             <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Relay Protocol</p>
                             <p className="text-base font-black text-slate-900 italic underline decoration-2 decoration-orange-500/20">{selectedTx.paymentMethod || 'Manual Entry'}</p>
                          </div>
                       </div>
                       ...
                       <div className="space-y-4">
                          <div className="flex justify-between items-center">
                             ...
                             <code className="text-[11px] font-black text-slate-900 bg-slate-100 px-3 py-1 rounded-lg">#{selectedTx.payHereOrderId || 'MB-INTERNAL'}</code>
                          </div>
                          <div className="flex justify-between items-center">
                             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Process Timeline</span>
                             <span className="text-[11px] font-black text-slate-900 italic">{new Date(selectedTx.createdAt).toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between items-center">
                             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Internal Status</span>
                             <span className={`px-4 py-1 text-[10px] font-black uppercase rounded-lg border italic ${statusBadgeStyle[selectedTx.status]}`}>{selectedTx.status} Hub</span>
                          </div>
                       </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="p-8 bg-slate-50 rounded-[40px] border border-dashed border-slate-200 min-h-[120px]">
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Relay Justification / Notes</p>
              ...
              <div className="p-12 bg-slate-50/50 border-t border-slate-50 flex items-center justify-between">
                 <div className="flex items-center gap-4">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 max-w-xs leading-relaxed italic">
                      This record is secured via Institutional Identity Hub and cannot be manually modified Hub.
                    </p>
                 </div>
                 <button className="px-10 py-5 bg-slate-900 text-white rounded-3xl text-[10px] font-black uppercase tracking-widest shadow-2xl shadow-slate-900/40 hover:bg-orange-600 transition-all flex items-center gap-3 italic">
