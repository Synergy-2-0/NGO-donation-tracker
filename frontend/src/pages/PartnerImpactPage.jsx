    ...
    return (
      <div className="max-w-md mx-auto py-24 text-center space-y-6">
        ...
        <div className="space-y-2">
          <h3 className="text-xl font-extrabold text-slate-800">Intelligence Restricted</h3>
          <p className="text-slate-500 text-sm font-medium leading-relaxed">
            {error || 'Impact analytics are restricted to verified strategic partners with active history.'}
          </p>
        </div>
        <Link to="/partners" className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-extrabold uppercase tracking-widest hover:bg-brand-red transition-all shadow-xl shadow-slate-900/10 active:scale-95">
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-16 animate-fadeIn text-left">
      ...
      <div className="relative overflow-hidden bg-slate-900 rounded-[32px] p-8 md:p-10 shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-[.2em] text-emerald-400 mb-2">Performance Intelligence</p>
          ...
          <Link to={`/partners/${id}`} className="px-5 py-3 bg-white/5 hover:bg-white/10 text-white text-[10px] font-extrabold uppercase tracking-widest rounded-xl transition-all border border-white/10 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[40px] border border-slate-100 shadow-sm p-8 text-left space-y-8">
          <div className="flex items-center gap-3">
             ...
             <h3 className="text-[10px] font-extrabold uppercase tracking-widest text-slate-500">Domain Alignment Nodes</h3>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-4">
              <p className="text-[10px] font-extrabold text-slate-300 uppercase tracking-widest">CSR Strategic Focus</p>
              {impactData.csrFocus?.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {impactData.csrFocus.map(v => (
                    <span key={v} className="px-4 py-2 bg-slate-50 border border-slate-100 text-slate-600 text-[10px] font-extrabold uppercase tracking-widest rounded-xl transition-all hover:bg-slate-100">
                      {String(v).replace(/_/g, ' ')}
                    </span>
                  ))}
                </div>
              ) : <p className="text-sm font-medium text-slate-300 ">No nodes identified</p>}
            </div>

            <div className="space-y-4">
              <p className="text-[10px] font-extrabold text-slate-300 uppercase tracking-widest">Global SDG Synergy</p>
              {impactData.sdgGoals?.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {impactData.sdgGoals.map(v => (
                    <span key={v} className="px-4 py-2 bg-emerald-50 border border-emerald-100 text-emerald-600 text-[10px] font-extrabold uppercase tracking-widest rounded-xl transition-all hover:bg-emerald-100">
                      SDG {v}
                    </span>
                  ))}
                </div>
              ) : <p className="text-sm font-medium text-slate-300 ">No synergy mapped</p>}
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-[40px] p-8 text-white relative overflow-hidden flex flex-col justify-between text-left group">
           ...
           <div className="relative z-10">
              ...
              <h3 className="text-xl font-extrabold tracking-tight mb-3">Verified Delivery Node</h3>
           ...
           <div className="relative z-10 flex flex-col gap-3">
              <div className="flex justify-between items-end border-b border-white/5 pb-4">
                 <div>
                    <p className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">History</p>
                    <p className="text-lg font-extrabold text-white">{impactData.completedAgreements ?? 0} Completed</p>
                 </div>
                 <FiClock className="text-slate-700 mb-1" />
              </div>
              <button className="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-slate-900 font-extrabold text-[10px] uppercase tracking-[.2em] rounded-2xl transition-all active:scale-95 shadow-xl shadow-emerald-500/10">
