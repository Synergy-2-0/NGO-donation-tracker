        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-10 animate-fadeIn">
              ...
              <div className="text-center space-y-3">
                 <h2 className="text-2xl font-extrabold  tracking-tighter text-slate-900">{t('ai_match.loading.title')}</h2>
                 <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.3em] animate-pulse">{t('ai_match.loading.subtitle')}</p>
              </div>
            </div>
        );
    }
        ...
        <div className="max-w-7xl mx-auto space-y-12 pb-20 animate-fadeIn text-left">
            ...
            <section className="relative overflow-hidden bg-white border border-slate-100 rounded-[40px] p-10 md:p-14 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-12">
                ...
                <div className="space-y-6 flex-1 relative z-10">
                    <div className="inline-flex items-center gap-3 px-4 py-2 bg-slate-50 border border-slate-100 rounded-full">
                       ...
                       <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest  leading-none">{t('ai_match.hero.badge')}</span>
                    </div>
                    <h1 className="text-5xl md:text-6xl font-extrabold text-slate-900 tracking-tighter  leading-none">
                    ...
                    >
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <div className="text-right">
                                <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest leading-none mb-1">{t('ai_match.grid.synergy')}</p>
                                <p className="text-3xl font-extrabold  tracking-tighter text-slate-900">{mission.synergyScore}%</p>
                            </div>
                        </div>
                        <h3 className="text-xl font-extrabold text-slate-900  tracking-tight leading-tight mb-2 pr-4">{mission.title}</h3>
                        <p className="text-slate-400 text-[11px] font-extrabold uppercase tracking-widest mb-4">{t('ai_match.grid.ngo')}: {mission.createdBy?.name || 'Verified Alliance'}</p>
                        ...
                        <div className="flex items-center justify-between mt-auto pt-6 border-t border-slate-50">
                            ...
                            <button 
                                ...
                                className="text-tf-primary text-[10px] font-extrabold uppercase tracking-widest flex items-center gap-2 hover:translate-x-2 transition-transform"
                            >
            ...
            <div className="px-4">
                <div className="bg-slate-50 border border-slate-100 rounded-[40px] p-10 flex flex-col items-center justify-center space-y-6 text-center lg:text-left lg:flex-row lg:justify-between lg:px-20 relative overflow-hidden min-h-[500px]">
                    ...
                    <h3 className="text-3xl font-extrabold text-slate-900  tracking-tight leading-none">{t('ai_match.map.title')}</h3>
                    <p className="text-slate-500 text-sm font-medium  max-w-sm mb-8">{t('ai_match.map.subtitle')}</p>
                    ...
                    <div className="p-6 bg-white rounded-3xl shadow-xl border border-slate-100 max-w-xs">
                        <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2">{t('ai_match.map.density')}</p>
                        <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div className="h-full bg-tf-primary w-3/4" />
                        </div>
                        <p className="text-[9px] font-bold text-slate-500 mt-2 ">{t('ai_match.map.coverage', { percent: 75 })}</p>
                    </div>
                    <button 
                        onClick={() => navigate('/transparency')}
                        className="px-10 py-5 bg-slate-900 text-white border border-slate-200 rounded-3xl text-[10px] font-extrabold uppercase tracking-widest hover:bg-orange-600 transition-all flex items-center justify-center gap-3 shadow-xl group"
                    >
