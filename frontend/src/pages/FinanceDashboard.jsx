  ...
  if (summaryLoading && !summary) return (
    <div className="min-h-[80vh] flex items-center justify-center">
      <div className="flex flex-col items-center gap-6 text-left">
        <LoadingSpinner />
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 animate-pulse">Decrypting Ledger Hub...</p>
      </div>
    </div>
  );
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-16 text-left">
          <div className="space-y-4">
            ...
            >
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t('finance.transparency_hub')}</span>
            </motion.div>
            <h1 className="text-6xl font-black text-slate-900 tracking-tighter leading-none italic">
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={fetchReport}
              className="px-8 py-4 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:border-orange-500 transition-all flex items-center gap-3 text-slate-600 hover:text-orange-600 shadow-sm"
            >
            ...
            <motion.button
              ...
              className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-orange-600 transition-all shadow-2xl shadow-slate-900/20 flex items-center gap-3"
            >
              ...
              <div className="space-y-4">
                ...
                <div className="text-left">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-600 text-[8px] font-black rounded">{stat.trend}</span>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="premium-surface p-10 text-left">
              <div className="flex items-center justify-between mb-12">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-black text-slate-900 tracking-tight leading-none italic">Mission-Specific Deployment</h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 italic">Granular tracking of campaign-linked capital Hub</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {summary?.allocationsByCampaign?.length > 0 ? (
                      ...
                      <div key={item._id} className="p-6 bg-slate-50/50 rounded-3xl border border-slate-100 hover:border-indigo-200 transition-all group">
                        <div className="flex justify-between items-start mb-6">
                          <div className="space-y-1">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Mission Title</p>
                            <p className="text-sm font-black text-slate-900 line-clamp-1 group-hover:text-indigo-600 transition-colors italic">{item.title}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">{efficiency}% Deployed</p>
                          </div>
                        </div>
                        <div className="space-y-4">
                          <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-slate-400">
                            <span>Allocated</span>
                            <span className="text-slate-900 italic">LKR {item.totalAllocated.toLocaleString()}</span>
                          </div>
                  ...
                  <div className="col-span-2 py-20 flex flex-col items-center grayscale opacity-30">
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">No campaign-linked allocations detected Hub</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="premium-surface p-10 text-left">
              <div className="flex items-center justify-between mb-12">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-black text-slate-900 tracking-tight leading-none italic">{t('finance.allocation_breakdown')}</h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 italic">{t('finance.by_category')}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-8">
                {summary?.allocationsByCategory?.length > 0 ? (
                      ...
                      <div key={item._id} className="group/item">
                        <div className="flex justify-between items-end mb-3">
                          <div className="flex items-baseline gap-3">
                            <span className="text-4xl font-black text-slate-200 group-hover/item:text-orange-200 transition-colors italic leading-none">0{idx + 1}</span>
                            <div>
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 italic leading-none">{item._id}</p>
                              <p className="text-2xl font-black text-slate-900 tabular-nums leading-none">LKR {Number(item.total).toLocaleString()}</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="text-[10px] font-black text-orange-500 uppercase tracking-widest">{percentage}% Usage</p>
                          </div>
                        </div>
                  ...
                  <div className="py-32 flex flex-col items-center justify-center grayscale opacity-30 border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/50">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 italic">Waiting for deployment data...</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="premium-surface overflow-hidden text-left">
              <div className="p-10 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-black text-slate-900 tracking-tight leading-none italic">{t('finance.deployment_log')}</h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 italic">{t('finance.audit_sync')}</p>
                  </div>
                </div>
              </div>
                    ...
                    {summary?.recentAllocations?.length > 0 ? (
                      summary.recentAllocations.map(alloc => (
                        <tr key={alloc._id} className="group hover:bg-slate-50/50 transition-colors">
                          <td className="px-10 py-8">
                            <p className="text-xs font-bold text-slate-500 italic">{new Date(alloc.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                          </td>
                          <td className="px-10 py-8">
                            <span className="px-3 py-1 bg-slate-100 text-slate-900 rounded-lg text-[10px] font-black uppercase tracking-widest group-hover:bg-slate-900 group-hover:text-white transition-all italic">
                              {alloc.category}
                            </span>
                          </td>
                          <td className="px-10 py-8">
                            <p className="text-sm font-black text-slate-900 tabular-nums italic">LKR {alloc.amount.toLocaleString()}</p>
                          </td>
                      ...
                      <tr>
                        <td colSpan="4" className="px-10 py-24 text-center">
                          ...
                          <p className="text-[10px] font-black uppercase tracking-widest text-slate-300 italic">Financial integrity check passed. No deployments found.</p>
                        </td>
                      </tr>
                    )}
                  </tbody>
          ...
          <div className="space-y-8 text-left">
            <div className="premium-surface p-8 bg-slate-900 text-white relative overflow-hidden">
              ...
              <h4 className="text-xl font-black mb-2 tracking-tight italic leading-none">Treasury Compliance</h4>
              ...
              <div className="mt-8 pt-8 border-t border-white/10 flex items-center justify-between">
                <div>
                  <p className="text-[8px] font-black uppercase tracking-widest text-slate-500 mb-1 italic">Risk Score</p>
                  <p className="text-lg font-black text-emerald-400 underline decoration-2 decoration-emerald-400/30 italic leading-none">LOW / SECURE</p>
                </div>
                <div className="text-right">
                  <p className="text-[8px] font-black uppercase tracking-widest text-slate-500 mb-1 italic">Ledger status</p>
                  <p className="text-lg font-black italic leading-none">VERIFIED</p>
                </div>
              </div>
            </div>

            <div className="premium-surface p-8 space-y-6 border-dashed border-2 border-slate-100 bg-transparent text-left">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2 italic">
              ...
              <div className="space-y-4">
                ...
                <p className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.2em] italic">Generated by Synergy AI Advisor Hub</p>
              </div>
            </div>
          </div>
        </div>
            ...
            >
              <div className="p-10 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-4xl font-black text-slate-900 tracking-tighter italic leading-none">Institutional Audit Report</h3>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-3 italic leading-none">Verified Financial Transparency Dashboard Hub</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowComplianceModal(false)}
              ...
              <div className="p-10 grid grid-cols-1 lg:grid-cols-3 gap-10 max-h-[60vh] overflow-y-auto">
                <div className="lg:col-span-2 space-y-10">
                  {fetchingReport ? (
                    <div className="py-20 flex flex-col items-center gap-4 text-slate-300">
                      <LoadingSpinner />
                      <p className="text-[10px] font-black uppercase tracking-widest italic">Scanning global audit nodes...</p>
                    </div>
                  ) : (
                    <>
                      <div className="grid grid-cols-2 gap-6">
                        <div className="p-8 bg-slate-900 rounded-3xl text-white relative overflow-hidden group">
                          ...
                          <p className="text-[8px] font-black uppercase tracking-widest text-slate-500 mb-2 italic">Institutional Integrity Score</p>
                          <h4 className="text-6xl font-black text-orange-500 mb-1 italic leading-none">{transparencyReport?.score || '--'}</h4>
                          <p className="text-xs font-bold text-slate-400 italic">Class A Certified Hub</p>
                        </div>
                        <div className="p-8 bg-emerald-50 rounded-3xl border border-emerald-100">
                          <FiCheckCircle className="text-emerald-500 mb-4" size={32} />
                          <p className="text-[10px] font-black text-emerald-800 uppercase tracking-widest mb-1 italic leading-none">Audit Status</p>
                          <h4 className="text-2xl font-black text-emerald-900 uppercase tracking-tight italic leading-none">Verified Secure</h4>
                        </div>
                      </div>

                      <div className="space-y-6">
                        <h5 className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2 italic leading-none">
                          ...
                          {transparencyReport?.auditLogs?.slice(0, 5).map(log => (
                            <div key={log._id} className="p-5 bg-slate-50/50 border border-slate-100 rounded-2xl flex items-center justify-between group hover:bg-white transition-all">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <p className="text-xs font-black text-slate-900 uppercase tracking-widest leading-none mb-1 italic">{log.action} {log.entityType}</p>
                                  <p className="text-[10px] font-bold text-slate-400 italic leading-none">Performed by {log.changedBy?.name || 'System Registry'}</p>
                                </div>
                              </div>
                              <p className="text-[10px] font-black text-slate-300 tabular-nums uppercase italic">{new Date(log.createdAt).toLocaleTimeString()}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                <div className="space-y-8 text-left">
                  <div className="p-8 bg-slate-50 rounded-3xl space-y-6">
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 italic">Compliance Indicators Hub</p>
                    <div className="space-y-4">
                      {[
                        ...
                        <div key={i} className="flex justify-between items-center pb-3 border-b border-slate-200">
                          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic">{item.label}</span>
                          <span className="text-[10px] font-black text-slate-900 italic">{item.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <button className="w-full py-5 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-orange-600 transition-all flex items-center justify-center gap-3 italic font-serif">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-10 border-b border-slate-50 bg-slate-50/50 relative">
                <h3 className="text-4xl font-black text-slate-900 tracking-tighter italic leading-none">{t('finance.allocate_capital')}</h3>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2 flex items-center gap-2 italic">
              ...
              {localError && (
                <div className="mx-10 mt-6 p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center gap-4 animate-shake">
                  ...
                  <p className="text-xs font-bold text-rose-700 italic">{localError}</p>
                </div>
              )}

              <form onSubmit={handleAllocate} className="p-10 space-y-8">
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 italic">Source Capital Stream</label>
                  <div className="relative">
                    <select
                      required
                      value={allocationData.transactionId}
                      onChange={(e) => setAllocationData({ ...allocationData, transactionId: e.target.value })}
                      className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-5 text-sm font-bold appearance-none focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all cursor-pointer italic"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 italic">Mission Sector</label>
                    <select
                      required
                      value={allocationData.category}
                      onChange={(e) => setAllocationData({ ...allocationData, category: e.target.value })}
                      className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all italic"
                    >
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 italic">Capital Amount (LKR)</label>
                    <input
                      type="number"
                      required
                      min="1"
                      ...
                      className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all tabular-nums italic"
                    />
                  </div>
                </div>

                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 italic">Strategic Justification / Purpose</label>
                  <textarea
                    required
                    ...
                    rows="4"
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all resize-none italic"
                  />
                </div>
                ...
                <div className="pt-6 flex gap-6">
                  <button
                    type="button"
                    onClick={() => setShowModal(false)}
                    className="flex-1 py-5 bg-slate-50 text-slate-500 text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-slate-100 transition-all italic border border-slate-100"
                  >
                    Abort Deployment
                  </button>
                  <button
                    type="submit"
                    disabled={submitting}
                    className="flex-1 py-5 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-orange-600 transition-all shadow-xl shadow-slate-900/10 italic disabled:opacity-50 flex items-center justify-center gap-3"
                  >
                    {submitting ? (
                      <>
