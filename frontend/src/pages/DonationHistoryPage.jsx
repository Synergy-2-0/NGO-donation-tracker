      ...
      <div className="bg-[#0F172A] rounded-[3.5rem] p-12 text-white relative overflow-hidden shadow-2xl border border-white/5">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-end justify-between gap-12">
           <div className="space-y-6">
              <div className="space-y-4">
                 <div className="flex items-center gap-3">
                    ...
                    <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.6em] leading-none italic">{t('donation_history.header_badge')}</p>
                 </div>
                 <h2 className="text-4xl md:text-5xl font-bold tracking-tight leading-none italic">
                    {t('donation_history.title_1')} <span className="text-tf-primary not-italic">{t('donation_history.title_2')}</span>
                 </h2>
              </div>
           ...
           <div className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-10 flex flex-col gap-2 min-w-[300px] group hover:bg-white/10 transition-colors duration-500 shadow-2xl relative overflow-hidden">
              ...
              <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.4em] leading-none italic z-10">{t('donation_history.total_impact_label')}</p>
              <h4 className="text-3xl font-bold text-white tracking-tight tabular-nums z-10 leading-none italic">LKR {Number(transactions.reduce((sum, tx) => tx.status === 'completed' ? sum + Number(tx.amount) : sum, 0)).toLocaleString()}</h4>
      ...
      {!donorProfile ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-slate-50 rounded-[4rem] p-20 border border-slate-100 flex flex-col md:flex-row items-center gap-16 italic">
            ...
            <div className="space-y-6 text-center md:text-left">
                ...
                <p className="text-base text-slate-400 leading-relaxed max-w-xl font-medium italic">{t('donation_history.profile_verification.desc')}</p>
            </div>
        </motion.div>
      ) : transactions.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white rounded-[4rem] border border-slate-100 p-40 text-center space-y-8 shadow-sm relative overflow-hidden italic">
           ...
           <div className="space-y-4">
              <h4 className="text-3xl font-black text-slate-900 tracking-tight italic leading-none">{t('donation_history.no_history.title')}</h4>
              <p className="text-[11px] text-slate-500 font-black uppercase tracking-[0.5em] italic leading-relaxed">{t('donation_history.no_history.desc')}</p>
           </div>
        </motion.div>
      ) : (
        <div className="space-y-12">
          ...
          {pledges.length > 0 && (
            <section className="space-y-8 italic">
               <div className="flex items-center gap-4 px-6">
                  ...
                  <h3 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.5em] italic">{t('donation_history.active_commitments')}</h3>
                  <div className="h-px flex-1 bg-slate-100" />
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {pledges.map((p) => (
                    <motion.div 
                      ...
                      className="bg-slate-50/50 rounded-[2.5rem] p-10 border border-slate-100 shadow-sm relative overflow-hidden group hover:bg-white hover:shadow-xl transition-all duration-700 italic"
                    >
                       <div className="absolute top-0 right-0 w-32 h-32 bg-tf-primary/5 blur-[50px] -mr-16 -mt-16" />
                       <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                          <div className="space-y-4">
                             <div className="flex items-center gap-3">
                                <span className="px-5 py-1.5 bg-tf-primary/10 text-tf-primary border border-tf-primary/20 rounded-xl text-[9px] font-black uppercase tracking-widest leading-none italic">{t(`pledge_modal.freq.${p.frequency}`)} {t('donation_history.card.strategy_suffix')}</span>
                          ...
                          <div className="flex items-center gap-6">
                             <div className="text-right">
                                <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1">{t('donation_history.card.status_label')}</p>
                                <p className="text-sm font-black text-tf-accent italic uppercase leading-none">{t('donation_history.card.active_status')}</p>
                             </div>
          ...
          <section className="space-y-8 italic">
            <div className="flex items-center gap-4 px-6">
               <div className="h-px flex-1 bg-slate-100" />
               <h3 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.5em] italic">{t('donation_history.impact_registry')}</h3>
               <div className="h-px flex-1 bg-slate-100" />
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              {transactions.map((tx) => (
                <motion.div 
                  ...
                  className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm hover:shadow-3xl transition-all duration-700 group flex flex-col md:flex-row md:items-center justify-between gap-10 overflow-hidden relative italic"
                >
                  ...
                  <div className="flex items-center gap-10 relative z-10">
                     ...
                     <div className="space-y-3">
                        <div className="flex items-center gap-4">
                           ...
                           <p className="text-[10px] font-black text-slate-300 font-mono tracking-[0.2em] uppercase italic">{t('donation_history.card.ref_id')} {tx._id.slice(-8).toUpperCase()}</p>
                        </div>
                        <h4 className="text-2xl font-bold text-slate-900 tracking-tight leading-none italic tabular-nums">LKR {Number(tx.amount).toLocaleString()}</h4>
                     </div>
                  </div>

                  <div className="flex flex-wrap items-center gap-16 relative z-10">
                     <div className="space-y-2">
                        <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.4em] leading-none italic mb-1">{t('donation_history.card.method_label')}</p>
                        <p className="text-base font-black text-slate-700 italic capitalize leading-none tracking-tight">{tx.paymentMethod}</p>
                     </div>
                     <div className="space-y-2">
                        <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.4em] leading-none italic mb-1">{t('donation_history.card.project_label')}</p>
                        <p className="text-base font-black text-slate-900 italic leading-none tracking-tight">{tx.campaignId?.title || t('donation_history.card.general_fund')}</p>
                     </div>
                  </div>
