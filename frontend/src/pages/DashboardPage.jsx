  ...
  return (
    <div className="bg-white rounded-[3.5rem] border border-slate-100 p-12 shadow-sm hover:shadow-2xl transition-all duration-700 group relative overflow-hidden flex flex-col justify-between h-72">
      ...
      <div className="relative z-10 space-y-3">
        ...
        <p className="text-4xl lg:text-5xl font-black tracking-tighter text-tf-purple group-hover:text-tf-pink transition-colors tabular-nums">{value}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-12 animate-fade-in max-w-[1700px] mx-auto pb-24 font-sans selection:bg-tf-pink selection:text-white">
      ...
      <div className="relative p-12 lg:p-20 bg-tf-purple rounded-[4rem] overflow-hidden shadow-2xl group text-white border border-white/5">
        ...
        <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-tf-pink/10 blur-[130px] -mr-40 -mb-40 opacity-40 animate-pulse" />
        <div className="relative z-10 space-y-8 max-w-4xl">
           ...
           <h1 className="text-5xl lg:text-7xl font-black tracking-tighter uppercase italic leading-none truncate max-w-full italic tracking-tight">
              Hello, <span className="text-tf-pink">{user?.name?.split(' ')[0] || 'Friend'}</span>
           </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="bg-[#0f041a] rounded-[4rem] p-16 shadow-2xl relative overflow-hidden group text-white border border-white/5 space-y-12">
          <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-tf-pink/10 to-transparent pointer-events-none" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 relative z-10">
            {[
              { 
                ...
                icon: (
                  <svg className="w-5 h-5 text-tf-pink" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                ...
                className="flex flex-col items-start p-8 rounded-[2.5rem] bg-white/5 border border-white/5 group/item hover:bg-white hover:border-tf-pink hover:shadow-2xl transition-all text-left transform active:scale-95"
              >
        ...
        <div className="bg-white rounded-[4rem] border border-slate-100 p-16 shadow-sm space-y-12 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-64 h-64 bg-tf-pink/5 blur-[80px] -mr-32 -mt-32" />
          <div className="space-y-1 relative z-10">
          ...
          <div className="space-y-8 relative z-10">
            {donorProfile?.preferredCauses?.length > 0 ? (
              donorProfile.preferredCauses.map(cause => (
                <div key={cause} className="group/bar cursor-default">
                  <div className="flex justify-between items-end mb-3 px-2">
                    <p className="text-[11px] font-black text-tf-purple uppercase tracking-[0.2em] italic group-hover/bar:text-tf-pink transition-colors">#{cause.toUpperCase()}</p>
                  ...
                  <div className="h-1.5 bg-slate-50 border border-slate-100 rounded-full overflow-hidden p-0.5">
                    <div className="h-full bg-tf-purple w-full rounded-full transition-all duration-[1500ms] group-hover/bar:bg-tf-pink group-hover/bar:shadow-[0_0_15px_rgba(230,0,126,0.3)]" />
                  </div>
                </div>
              ))
            ) : (
              <div className="py-24 text-center space-y-6">
                ...
                <button onClick={() => navigate('/profile')} className="text-[12px] font-black text-tf-pink uppercase tracking-widest hover:underline underline-offset-8">Update Focus Hub</button>
              </div>
            )}
          </div>
        </div>
