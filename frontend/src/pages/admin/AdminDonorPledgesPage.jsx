        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-10 text-left">
          ...
          <div className="flex gap-4 shrink-0">
             <div className="bg-white/5 border border-white/10 backdrop-blur-xl px-8 py-6 rounded-2xl space-y-1 text-left">
                <p className="text-[10px] font-extrabold text-white/30 uppercase tracking-widest">Total Pledges</p>
                <p className="text-4xl font-extrabold text-tf-primary tabular-nums">{pledges.length}</p>
             </div>
             <div className="bg-white/5 border border-white/10 backdrop-blur-xl px-8 py-6 rounded-2xl space-y-1 text-left">
                <p className="text-[10px] font-extrabold text-white/30 uppercase tracking-widest">Active Commits</p>
                <p className="text-4xl font-extrabold text-emerald-500 tabular-nums">{pledges.filter(p => p.status === 'active').length}</p>
             </div>
          </div>
        </div>
      </section>
              ...
              <table className="w-full">
                <thead>
                  <tr className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 border-b border-slate-50">
                    <th className="px-6 pb-4 text-left font-extrabold">Community Supporter</th>
                    <th className="px-6 pb-4 text-left font-extrabold">Commitment Amount</th>
                    <th className="px-6 pb-4 text-center font-extrabold">Frequency</th>
                    <th className="px-6 pb-4 text-center font-extrabold">Sync Status</th>
                    <th className="px-6 pb-4 text-right font-extrabold">Actions</th>
                  </tr>
                </thead>
                  ...
                  {paginated.length > 0 ? paginated.map((pledge, idx) => (
                    ...
                    >
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-slate-100 text-slate-500 rounded-xl flex items-center justify-center font-extrabold text-sm group-hover/row:bg-tf-primary group-hover/row:text-white transition-all">
                             {pledge.donorId?.userId?.name?.charAt(0) || 'D'}
                          </div>
                          <div>
                            <p className="text-base font-extrabold text-slate-900 group-hover/row:text-tf-primary transition-colors leading-tight mb-0.5">{pledge.donorId?.userId?.name || 'Verified Member'}</p>
                      ...
                      <td className="px-6 py-5">
                        <div className="space-y-0.5 group/money">
                           <p className="text-lg font-extrabold text-slate-950 tracking-tight tabular-nums group-hover/row:text-tf-primary transition-colors leading-none">
                      ...
                      <td className="px-6 py-5 text-center">
                        <span className="px-4 py-1.5 bg-slate-950 text-white rounded-xl text-[9px] font-extrabold uppercase tracking-widest group-hover/row:bg-tf-primary transition-colors ">
                           {pledge.frequency?.toUpperCase() || 'ONE-TIME'}
                        </span>
                      </td>
                      <td className="px-6 py-5 text-center">
                         <span className={`px-4 py-1.5 rounded-full text-[9px] font-extrabold uppercase tracking-widest border transition-all duration-500 ${statusColor[pledge.status] || 'bg-white text-slate-300'}`}>
                            {pledge.status} Hub
                         </span>
                      </td>
                      <td className="px-6 py-5 text-right">
                         <div className="flex items-center justify-end gap-2">
                            <button 
                              ...
                              className="px-5 py-2.5 bg-white border border-slate-100 rounded-xl text-[10px] font-extrabold uppercase tracking-widest text-slate-500 hover:bg-slate-950 hover:text-white hover:border-slate-950 transition-all shadow-sm active:scale-95 flex items-center gap-2"
                            >
                    ...
                    <tr>
                      <td colSpan={5} className="py-24 text-center">
                         ...
                         <h4 className="text-sm font-extrabold text-slate-900 uppercase tracking-widest">No pledges found</h4>
