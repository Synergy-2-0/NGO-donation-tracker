   ...
   return (
     ...
     >
       <div className="relative aspect-[4/3] overflow-hidden">
          ...
          <div className="absolute top-6 left-6 px-4 py-2 bg-white/90 backdrop-blur-md rounded-2xl text-[9px] font-black text-slate-900 uppercase tracking-widest shadow-xl">
          ...
          <div className="space-y-4 pt-8 border-t border-slate-50 relative">
             <div className="flex justify-between items-end">
                <div className="space-y-1">
                   <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest italic leading-none mb-1">Raised So Far</p>
                   <p className="text-2xl font-black text-slate-900 tracking-tighter italic">LKR {(cause.raisedAmount || 0).toLocaleString()}</p>
                </div>
                <div className="text-right">
                   <p className="text-3xl font-black text-tf-primary tracking-tighter leading-none italic">{pct}%</p>
                </div>
             </div>
             ...
             <div className="flex justify-between items-center text-[9px] font-black text-slate-300 uppercase tracking-widest italic">
                <span>Goal: LKR {cause.goalAmount.toLocaleString()}</span>
                {cause.endDate && (
                  <span className="text-tf-primary flex items-center gap-1.5 font-black uppercase">
    ...
    <div className="min-h-screen bg-white font-sans selection:bg-tf-primary selection:text-white pb-0 overflow-x-hidden pt-12">
      ...
      <section className="relative pt-48 pb-32 px-8 overflow-hidden group/hero">
        ...
        <div className="max-w-7xl mx-auto space-y-12 relative z-10 text-left">
           <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-4">
              ...
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.5em] leading-none italic">Verified Humanitarian Causes</p>
           </motion.div>
           <div className="space-y-6">
              <motion.h1 
                initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}
                className="text-7xl md:text-9xl font-black tracking-tighter leading-[0.85] italic lowercase text-slate-900"
              >
                Our <span className="text-tf-primary italic">Missions.</span>
              </motion.h1>
              <motion.p 
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
                className="text-xl md:text-2xl text-slate-500 max-w-4xl leading-relaxed italic border-t border-slate-100 pt-12 mt-12 font-medium"
              >
         ...
         {loading ? (
           ...
           <AnimatePresence mode="popLayout">
              {causes.length > 0 ? (
               ...
               >
                  <div className="w-20 h-20 bg-slate-50 rounded-3xl flex items-center justify-center text-slate-200 text-3xl font-black italic">!</div>
                  <div className="space-y-2">
                     <h3 className="text-2xl font-black text-slate-900 italic tracking-tight uppercase">No Active Missions</h3>
                     <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.4em] italic">Check back shortly for new humanitarian projects Hub</p>
                  </div>
                  <button onClick={() => window.location.reload()} className="px-10 py-5 bg-slate-900 text-white rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-tf-primary transition-all shadow-xl active:scale-95">Refresh Registry Hub</button>
               </motion.div>
              )}
           </AnimatePresence>
         )}
      ...
      <section className="py-32 bg-slate-900 text-white relative overflow-hidden text-center mx-4 md:mx-10 mb-10 rounded-[4rem] shadow-5xl border border-white/5">
         ...
         <div className="relative z-10 max-w-4xl mx-auto space-y-12 px-6">
            <h3 className="text-4xl md:text-6xl font-black italic tracking-tighter leading-tight lowercase">
               Create Lasting <span className="text-tf-primary italic">Change</span> With Your Support Hub.
            </h3>
            <p className="text-white/40 text-lg md:text-xl font-medium italic leading-relaxed max-w-2xl mx-auto">Join thousands of verified donors providing direct aid to humanitarian projects across the globe.</p>
            <div className="pt-8">
               <button onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})} className="px-16 py-6 bg-tf-primary text-white rounded-full text-[11px] font-black uppercase tracking-[0.4em] hover:bg-white hover:text-slate-900 transition-all shadow-5xl shadow-tf-primary/20 italic">Explore Causes Hub →</button>
            </div>
         </div>
      </section>
