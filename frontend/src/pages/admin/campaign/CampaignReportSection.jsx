            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
                <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-1">
                        <h3 className="text-3xl font-extrabold text-slate-950 tracking-tighter uppercase  leading-none">Strategy Completion</h3>
                        <p className="text-[10px] font-extrabold text-slate-300 uppercase tracking-[0.4em] leading-none ">Verified Tactical Outcome Log</p>
                    </div>
                </div>
            ...
            <div className="space-y-10 relative z-10">
                    ...
                    <div className="space-y-4">
                        {error && (
                            <div className="flex items-center gap-4 px-10 py-6 bg-rose-50 border border-rose-100 rounded-[2rem] shadow-sm">
                                ...
                                <div className="space-y-1">
                                   <p className="text-[10px] font-extrabold text-rose-400 uppercase tracking-widest leading-none ">Registry Alert</p>
                        ...
                        {success && (
                            <div className="flex items-center gap-4 px-10 py-6 bg-emerald-50 border border-emerald-100 rounded-[2rem] shadow-sm">
                                ...
                                <div className="space-y-1">
                                   <p className="text-[10px] font-extrabold text-emerald-400 uppercase tracking-widest leading-none ">Report Finalized</p>
                ...
                {loading ? (
                    <div className="py-20 flex flex-col items-center justify-center space-y-6">
                        ...
                        <p className="text-[10px] font-extrabold text-slate-300 uppercase tracking-[0.5em] animate-pulse">Synchronizing Outcomes...</p>
                    </div>
                ) : report ? (
                    ...
                    <div className="space-y-12">
                        <div className="flex items-center gap-6 px-10 py-8 bg-emerald-50/50 rounded-[3rem] border border-emerald-100 animate-in fade-in slide-in-from-top-4 duration-700">
                            ...
                            <div className="space-y-1">
                                <p className="text-[10px] font-extrabold text-emerald-600 uppercase tracking-[0.4em]  leading-none">Mission Successfully Concluded</p>
                                <p className="text-sm text-emerald-800 font-bold  tracking-tight">Outcome synchronized and verified on {new Date(report.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div className="bg-slate-50 rounded-[3rem] p-10 border border-slate-100 shadow-inner group/stat hover:bg-white transition-all duration-500">
                                <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.4em] mb-4 ">Total Capital Stabilized</p>
                                <p className="text-4xl font-extrabold text-tf-primary tracking-tighter  tabular-nums leading-none">LKR {Number(report.totalRaised).toLocaleString()}</p>
                            </div>
                            <div className="bg-slate-50 rounded-[3rem] p-10 border border-slate-100 shadow-inner group/stat hover:bg-white transition-all duration-500">
                                <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.4em] mb-4 ">Community Beneficiaries</p>
                                <p className="text-4xl font-extrabold text-indigo-500 tracking-tighter  tabular-nums leading-none">{Number(report.beneficiaries).toLocaleString()}</p>
                            </div>
                        </div>

                        <div className="space-y-4 px-4">
                            <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.4em] ">Strategic Executive Summary</p>
                            <p className="text-lg text-slate-700 font-bold leading-relaxed  tracking-tight">{report.summary}</p>
                        </div>

                        {report.evidence?.length > 0 && (
                            <div className="space-y-6">
                                <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.4em]  ml-4">Verification Artifacts</p>
                                <div className="flex flex-wrap gap-4">
                                    {report.evidence.map((url, i) => (
                                        ...
                                        >
                                            ...
                                            <p className="text-[11px] font-extrabold text-slate-600 group-hover/file:text-tf-primary uppercase tracking-widest transition-colors mb-0">Artifact_{i + 1}.LOG</p>
                                        </a>
                                    ))}
                                </div>
                            </div>
                    ...
                    <form onSubmit={handleSubmit} className="bg-slate-50 rounded-[3rem] p-12 border border-slate-100 shadow-inner space-y-10 group/form">
                        <div className="flex items-center gap-4 mb-2">
                             ...
                             <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.4em] ">Final Mission outcome Synchronization</p>
                        </div>

                        <div className="space-y-4">
                             <div className="flex items-center justify-between px-4">
                                <label className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.4em] ">Executive Impact Summary</label>
                                <button
                                    type="button"
                                    onClick={handleGenerateSummary}
                                    disabled={generatingSummary}
                                    className="px-4 py-2 bg-tf-primary/5 border border-tf-primary/10 rounded-full text-[9px] font-extrabold text-tf-primary uppercase tracking-[0.2em] hover:bg-tf-primary hover:text-white transition-all duration-500 flex items-center gap-3 active:scale-95 disabled:opacity-50 group/ai"
                                >
                                    {generatingSummary ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div className="space-y-4">
                                <label className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.4em] ml-4 ">Final Capital Stabilization (LKR)</label>
                            ...
                            <div className="space-y-4">
                                <label className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.4em] ml-4 ">Verified Life Contacts</label>
                                <div className="relative">
                                    <input
                                        type="number"
                                        name="beneficiaries"
                        ...
                        <div className="space-y-4">
                            <label className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.4em] ml-4 ">Verification Repository</label>
                            <label className="relative flex items-center gap-6 px-10 py-6 bg-white border border-dashed border-slate-200 rounded-[2rem] cursor-pointer hover:border-tf-primary hover:bg-tf-primary/5 transition-all duration-500 group/evidence overflow-hidden">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[9px] text-slate-400 font-extrabold uppercase tracking-widest ">Verification Logs, Completion Certificates, Impact Records</p>
                                </div>
                    ...
                    <div className="py-24 text-center group/empty">
                        ...
                        <h4 className="text-xl font-bold text-slate-400 tracking-tight ">Null Outcome Log</h4>
                        <p className="text-[10px] text-slate-300 font-extrabold uppercase tracking-[0.4em] mt-2 ">Strategic report initialization pending mission completion verification.</p>
                    </div>
                )}
            </div>
        </div>
