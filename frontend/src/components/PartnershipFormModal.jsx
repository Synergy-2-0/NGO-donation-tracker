...
export default function PartnershipFormModal({ onClose, onSave, initialCampaignId }) {
    ...
    const milestonesTotal = (autoMilestones ? getAutoGeneratedPhases() : milestones).reduce((sum, m) => sum + (Number(m.budget) || 0), 0);
    ...
    const isFormValid = (
        ...
        milestonesTotal <= agreementTotal &&
        (autoMilestones || milestones.every(m => m.title && m.budget >= 0 && m.dueDate))
    );
    ...
    const handleSubmit = async (e) => {
        ...
        // Validate milestones lifecycle Hub
        const agreementStart = new Date(form.startDate);
        const agreementEnd = new Date(form.endDate);

        for (const ms of finalMilestones) {
            if (ms.dueDate) {
                const msDate = new Date(ms.dueDate);
                if (msDate < agreementStart || msDate > agreementEnd) {
                    return setError(`Milestone "${ms.title}" date (${ms.dueDate}) must be within the partnership lifecycle.`);
                }
            }
        }
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 text-left">
            <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col animate-scaleIn">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="md:col-span-2 space-y-2 group">
                            ...
                            <input 
                                ...
                                className={`w-full bg-slate-50 border ${form.title && form.title.length < 5 ? 'border-rose-200 ring-rose-50' : 'border-slate-100'} rounded-2xl px-5 py-4 text-base font-extrabold text-slate-800 placeholder:text-slate-300 focus:outline-none focus:ring-4 focus:ring-tf-primary/5 transition-all outline-none `}
                            />
                            {form.title && form.title.length < 5 && (
                                <p className="text-[9px] font-extrabold text-rose-500 uppercase tracking-widest pl-2">Title must be at least 5 characters.</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-2 group">
                            ...
                            <input 
                                ...
                                onChange={e => setForm({...form, totalValue: Math.max(0, e.target.value)})}
                            ...
                            />
                            {milestonesTotal > agreementTotal && (
                                <p className="text-[9px] font-extrabold text-rose-500 uppercase tracking-widest pl-2">Total exceeded by { (milestonesTotal - agreementTotal).toLocaleString() } LKR</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-2 group">
                            ...
                            <input 
                                ...
                                className={`w-full bg-slate-50 border ${form.startDate && form.endDate && new Date(form.endDate) <= new Date(form.startDate) ? 'border-rose-200 ring-rose-50' : 'border-slate-100'} rounded-2xl px-5 py-4 text-sm font-extrabold text-slate-800 focus:outline-none focus:ring-4 focus:ring-tf-primary/5 transition-all outline-none `}
                            />
                            {form.startDate && form.endDate && new Date(form.endDate) <= new Date(form.startDate) && (
                                <p className="text-[9px] font-extrabold text-rose-500 uppercase tracking-widest pl-2">Must be after Start Date.</p>
                            )}
                        </div>
                                           ...
                                           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                               ...
                                               <div>
                                                   <input 
                                                        ...
                                                        value={ms.budget}
                                                        onChange={(e) => updateMilestone(idx, 'budget', Math.max(0, e.target.value))}
                                                        className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-xs font-bold text-slate-800 focus:border-tf-primary outline-none transition-all "
                                                   />
                                               </div>
                                               <div>
                ...
                <div className="p-10 border-t border-slate-50 bg-slate-50/50 flex gap-4 shrink-0">
                    ...
                    <button onClick={handleSubmit} disabled={fetching || saving || !isFormValid} className="flex-[2] px-8 py-5 bg-tf-primary text-white rounded-2xl text-[10px] font-extrabold uppercase tracking-[0.2em] shadow-2xl shadow-tf-primary/30 transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed ">
