  ...
  return (
    <div className="min-h-screen bg-white font-sans selection:bg-tf-primary selection:text-white pb-0 overflow-x-hidden">
      ...
      <section className="pt-48 pb-32 px-8 lg:px-24 bg-tf-secondary bg-gradient-to-b from-slate-50 to-white">
        <div className="max-w-[1400px] mx-auto text-center space-y-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <p className="text-[12px] font-bold text-tf-primary uppercase tracking-[0.6em] mb-6">Our Global Impact</p>
            <h1 className="text-6xl md:text-9xl font-bold font-display text-tf-dark tracking-tighter uppercase italic leading-[0.85]">
              Real <br /> <span className="text-tf-primary tracking-normal">Humanitarian</span> <br /> Outcomes.
            </h1>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-xl text-slate-500 font-serif font-medium leading-relaxed max-w-2xl mx-auto italic">
            Every contribution through TransFund is a direct investment into community resilience. We track every outcome with precision.
          </motion.p>
        </div>
      </section>
      ...
      <section className="py-24 max-w-[1400px] mx-auto px-8 grid grid-cols-2 lg:grid-cols-4 gap-16 text-center items-center divide-x-0 lg:divide-x divide-slate-100">
         {[
           { label: 'Total Giving', value: 'LKR 42.8M+', color: 'text-tf-dark' },
           { label: 'Verified Partners', value: '480+', color: 'text-tf-primary' },
           { label: 'Active Projects', value: '1,200+', color: 'text-tf-accent' },
      ...
      <section className="py-32 px-8 lg:px-24 bg-slate-50/20">
         <div className="max-w-[1400px] mx-auto space-y-24">
            <div className="text-center space-y-6">
               <p className="text-[12px] font-bold text-tf-primary uppercase tracking-[0.8em]">Impact Highlights</p>
               <h2 className="text-5xl md:text-7xl font-display font-medium text-tf-dark tracking-tight leading-[0.9]">Verified Success.</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 items-start">
               {[
                 { title: 'Clean Water Initiative', desc: 'Installed 45 deep-bore wells in Anuradhapura, providing permanent water security for 12,000 agricultural families.', metric: '12,000 Families' },
                 { title: 'Digital Education Hubs', desc: 'Upgraded 8 rural schools in Jaffna with high-speed digital tools and modern libraries, impacting 3,500 children.', metric: '3,500 Children' },
                 { title: 'Community Health', desc: 'Distributed emergency surgical kits and life-saving meds to 30 primary clinics in rural Matara.', metric: '30 Clinics' }
               ].map((item, idx) => (
                 <motion.div 
                   key={idx}
